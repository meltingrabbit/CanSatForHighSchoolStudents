...
float readTemperature() {
	...
	barometer.t_fine = var1 + var2;

	float T = (barometer.t_fine * 5 + 128) >> 8;
	return T / 100;
}
...
float readPressure() {
	...
	// Must be done first to get the barometer.t_fine variable set up
	readTemperature();

	int32_t adc_P = read24(BMP280_REGISTER_PRESSUREDATA);
	adc_P >>= 4;

	var1 = ((int64_t)barometer.t_fine) - 128000;
