...
void setup() {
 ...
 Serial.println(F("sd done"));
	BTH_Init();			// 気圧・温度計を初期化
 Serial.println(F("bth done"));
	IMU_Init();			// 9軸センサの初期化
  Serial.println(F("imu done"));
	GPS_Init();			// GPSの初期化
...
//  // 列の文字列
  String row = //String(pressure) + "hello"
//       + String(humidity) + "hello"
//       + String(temperature) + "hello"
	...
	SD_Write(row);		// SDカードに保存
  Serial.print(row); // シリアル出力
//  SD_Write(String(pressure) + ","
//    + String(humidity) + ","
//    + String(temperature) + ","
//    + String(acc_x) + ","
//    + String(acc_y) + ","
